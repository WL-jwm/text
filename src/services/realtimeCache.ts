/**
 * G-02 IDB 缓存与离线分析 — 实时数据缓存层
 *
 * 将实时监测数据持久化到 IndexedDB，支持：
 *   - 按通道+时间窗口存储读数
 *   - 离线历史查询（最近 N 条 / 时间范围查询）
 *   - 自动清理过期数据（保留最近 7 天）
 *   - 统计聚合（均值/极值/分位数）
 *   - 离线分析数据导出
 *
 * 数据库结构：
 *   DB: hebei-gw-realtime-cache
 *   Store: readings (keyPath: id, indexes: by-channel, by-timestamp)
 *   Store: stats (keyPath: id) — 预聚合统计
 *   Store: meta (keyPath: key) — 元数据（最后清理时间等）
 */
...
interface RealtimeCacheDBSchema extends DBSchema {
  readings: {
    key: string;
    value: CachedReading;
    indexes: {
...
const DB_NAME = 'hebei-gw-realtime-cache';
const DB_VERSION = 1;
const MAX_RETENTION_DAYS = 7;
const MAX_READINGS_PER_CHANNEL = 10000;
