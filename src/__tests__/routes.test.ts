  ...
  describe('页面文件存在性', () => {
    ...
    it('每个页面文件应对应一个路由定义', () => {
      ...
      const extraPages = pages
        ...
        .filter(name => !routeComponents.has(name) && !['NotFound', 'DataInsightInner', 'OverviewHelpers', 'timeSeriesUtils', 'timeSeriesSelectors', 'ForecastPanels', 'ExploitationTrendPanel', 'WaterLevelPanel', 'SubsidenceTrendPanel', 'QualityTrendPanel', 'SupplyStructurePanel', 'RadarComparePanel', 'CorrelationPanel', 'GovernancePanel'].includes(name));
